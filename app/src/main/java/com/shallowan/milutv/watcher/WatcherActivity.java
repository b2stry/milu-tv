...
import com.shallowan.milutv.hostlive.QuitRoomRequest;
...
import com.shallowan.milutv.widget.GiftSelectDialog;
...
public class WatcherActivity extends AppCompatActivity {
    ...
    private void updateTitleView() {
        ...
        // 添加自己的头像到titleView上。
        titleView.addWatcher(MiluApplication.getApplication().getSelfProfile());
                ...
                public void onSuccess(TIMMessage data) {
                    if (customCmd.getCmd() == Constants.CMD_CHAT_GIFT) {
                        ...
                        if (giftInfo.type == GiftInfo.Type.ContinueGift) {
                            giftRepeatView.showGift(giftInfo, repeatId, MiluApplication.getApplication().getSelfProfile());
                        } else if (giftInfo.type == GiftInfo.Type.FullScreenGift) {
                            //全屏礼物
                            giftFullView.showGift(giftInfo, MiluApplication.getApplication().getSelfProfile());
                        }
                    }
                }
